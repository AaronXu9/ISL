...
def read_sachs_all(folder_path):
    ...
    Returns:
        np.array containing all the sachs data
    """
    sachs_data = list()
    '''divide the sachs dataset into 14 environments according to the way the data was collected'''
    Y_label = []
...
def preprocess_sachs(folder_path, save_path, num_envs, normalization='standard'):
    """
        Args:
            folder_path: read sachs from the path
            save_path: save data in this path
            num_envs: number of environments to cluster the data into
        Returns:
            sachs dataset with different envs [num_envs, number of sample in each envs, num of features]
...
def read_sachs_to_envs(folder_path: str, num_envs: int, normalization: str, format='np'):
    """
    ...
    if num_envs==14:
        ...
        ''''Divide the data into two envs separated by the approach that data was collected'''
        ...
        for i in range(num_envs):
            X_envs[i], Y_label[i] = utils.preprocess_labeled_data(X_envs[i], Y_label[i], normalization)
        ...
        '''divide the sachs dataset into 3 environments created by clustering'''
        if not os.path.exists(f'./data/cluster/*3.csv'):
            X_envs = preprocess_sachs(folder_path, f'./data/cluster/', 3, normalization)
        else:
        ...
        '''divide the sachs dataset into 6 environments created by clustering'''
        if not os.path.exists(f'./data/cluster/*3.csv'):
            X_envs = preprocess_sachs(folder_path, f'./data/cluster/', 6, normalization)
        else:
...
def find_opt_k(x_raw):
    """
    Plots the WSS score and silhouse score for finding the optimal k for clustering X_raw
    Args:
        x_raw: np.array, raw data to be clustered
    """
...
def preprocess_BH(save_path, cluster):
    """
    Cluster the data and prerpocess it
    Args:
    ...
    Returns:
        [num_envs, number of sample in each envs, num of features]
    """
    np.set_printoptions(precision=3)
...
def cluster_Insurance(x, num_cluster, save_path=''):
    """
    Cluster X into num_clusters and return the data clustered into different environments.
    Args: 
        X: the data to be clustered
        num_cluster: the number of clusters 
        save_path: the path to save the clustered data file
    Returns:
...
def load_insurance(file_path, model_type='ISL'):
    ...
    Loads insurance dataset based on the model_type
        Args:
            ...
            model_type: the model that utilizes the loaded data
                        ISL requires train data from different envs 
                        other models require train data and test data
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='BH', help='the name of the dataset to be loaded')
    ...
    parser.add_argument('--num_cluster', type=int, default=3, help='number of clusters')

    args = parser.parse_args()
    return args


def load_data(dadataset_name, preprocess, num_envs):
    """
    load the dataset according to the instructinos
    Args:
        dadataset_name: name of the dataset to be loaded
    ...
    """
    if dadataset_name == 'sachs':
        return read_sachs_to_envs('./data/sachs_data', num_envs=num_envs, normalization=preprocess)
    
    elif dadataset_name == 'BH':
        x, x_tes = preprocess_BH('./data/BH/', num_envs)
        return 
    
    elif dadataset_name == 'Insurance':
...
if __name__ == '__main__':
    ...
    sachs_envs = read_sachs_to_envs(args.folder_path, args.num_cluster, args.normalization)
