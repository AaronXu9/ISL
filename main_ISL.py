""" Run Invariant Structure Learning (ISL) to discover DAG"""
...
def Swap(arr, start_index, last_index):
    '''
    Description: switch col start_index and last_index
    arr: numpy array
    return numpy array
    '''
    arr[:, [start_index, last_index]] = arr[:, [last_index, start_index]] # swap collum

def Swap_row_collum(arr, start_index, last_index):
    '''
    Description: switch col start_index and last_index
    arr: numpy array
    return numpy array
...
def main(args):
    ...
    # # Swap Y, we do not need swap label because when drawing, we will keep the original order.
    # swap_vertex_label(vertex_label, y_index)
    ...
    for i in range(len(W_est_origin_envs)):# for each environment
        Swap_row_collum(W_est_origin_envs[i], 0, y_index) # recover to original order
        if args.Only_Y_DAG: # for boston housing, we only care Y related DAG
            W_est_origin_envs[i][:, 1:] = np.zeros(W_est_origin_envs[i][:, 1:].shape)
        # rank W
    ...
    # model2 = nonlinear.NotearsMLP(dims=[d, args.hidden, 1], bias=True)
