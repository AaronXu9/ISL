...
It is also used for making predictions based on causal parents discovered by ISL
'''
import torch
...
def parse_args():
    ...
    parser.add_argument('--num_epoch', type=int, default=300, help='number of training epoches')
    parser.add_argument('--activation', type=str, default='relu', help='relu, sigmoid')
    parser.add_argument('--data_source', type = str, default='BH', help='BH, insurance')
    parser.add_argument('--normalization', type = str, default='standard', help='')
    parser.add_argument('--batch', type = int, default=256, help='')
    parser.add_argument('--causal', type = boolean, default=False, help='')
...
if __name__ == '__main__':
    ...
    if args.data_source == 'BH':
        X_train = np.loadtxt('./data/BH_train_test_trainparam/standard_BH_train.csv', delimiter=',')
        X_test = np.loadtxt('data/BH_train_test_trainparam/standard_BH_test.csv',  delimiter=',')
        activation = 'ReLU'

    elif args.data_source == 'Insurance':
        X_train = np.loadtxt('./data/Insurance/IRM/standard_Insurance_train.csv', delimiter=',')
        X_test = np.loadtxt('data/Insurance/IRM/standard_Insurance_test.csv',  delimiter=',')
        vertex_label = ['PropCost','GoodStudent', 'Age', 'SocioEcon','RiskAversion',
                        ...
                        'ThisCarCost', 'Theft', 'CarValue', 'HomeBase', 'AntiTheft', 'OtherCarCost',
                        'OtherCar', 'MedCost', 'Cushioning', 'Airbag', 'ILiCost', 'DrivHist'
                        ]
        activation = 'ReLU'

    elif args.data_source == 'synthetic':
        X_train = np.loadtxt(f'./output/synthetic/binary/{args.vars}/prob0123/X_train.csv', delimiter=',')
        X_test = np.loadtxt(f'./output/synthetic/binary/{args.vars}/prob0123/X_test_ID.csv', delimiter=',')
        activation = 'Sigmoid'
    ...
    if args.causal:
        ...
        y_train = torch.Tensor(X_train[:,0]) # prop cost or med
        x_test = torch.Tensor(X_test[:, 1:args.num_xy])

        y_test = torch.Tensor(X_test[:,0]) # porp cost or med
    ...
    net = MLP(n_feature=input_feature_dim, n_hidden=args.hidden, n_output=output_feature_dim, activation=activation)
    print(net)
    ...
    print(mean_squared_loss(x_test.shape[0], np.array(y_test.detach().cpu()), np.array(output[:,0].detach().cpu())))

    if args.vars:
        X_OOD_test = np.loadtxt(f'./output/synthetic/binary/{args.vars}/prob0123/X_test_OOD.csv', delimiter=',')
        print('ood_test:')
        if not args.causal:
        ...
        print(mean_squared_loss(X_OOD_test.shape[0], X_OOD_test[:,0], np.array(output[:,0].detach())))
